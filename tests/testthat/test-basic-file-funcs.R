...
test_that("get.thresholds.chunk generates thresholds for a data subset", {
	...
	if(all(file.exists(thresh.files))) {
		...
		expect_equal(thresholds.chunk.tmax.only, dat)
		
		lapply(thresholds.netcdf, nc_close)
	}
})

test_that("compute.indices.for.stripe computes climdex indexes for a subset", {
	## Define mappings and filenames.
	...
	if(length(input.files) > 0 && all(file.exists(input.files)) && all(file.exists(thresh.files))) {
		...
		res <- lapply(names(cdx[[1]]), function(x) {
					...
					expect_equal(d.input, d.comparison, tolerance=1e-5)
...
test_that("get.quantiles.for.stripe calculates quantiles for a single stripe", {
...
test_that("get.quantiles.object returns a valid quantile object for further processing", {
	skip("validation data unavailable")
	## Define mappings and filenames.
	thresholds.name.map <- c(tx10thresh="tx10thresh", tn10thresh="tn10thresh", tx90thresh="tx90thresh",
			tn90thresh="tn90thresh", r95thresh="r95thresh", r99thresh="r99thresh")
	thresh.files <- "correct_output/thresholds_monClim_CanESM2_historical_r1i1p1_1961-1990.nc"
...
test_that("get.northern.hemisphere.booleans determines which points are in the northern hemisphere", {
	skip("test data unavailable")
...
test_that("get.data reads data from a file and does unit conversions", {
	skip("test data unavailable")
	test.dir <- "test3/"
	variable.name.map <- c(tmax="tasmax", tmin="tasmin", prec="pr")
...
## FIXME: Needs proper test data. This is just a framework...
test_that("functions for reading a file work correctly", {
	skip("test data unavailable")
	test.dir <- "test3/"
	if(file.exists(test.dir)) {
		input.files <- list.files(test.dir, full.names=TRUE)
...
test_that("thresholds and indexes can be calculated", {
